...
$('#edit_img_title_rightBox').on('click',async function () {
   if (mode == 'EDIT') {
      ...
      $('#edit_img_title').text('편집');
      canvas = $('<canvas>').addClass("edit_photo_canvas");
      ...
      textArea();
        ...
        for (var i = 0; i < fileLength; i++) {
            imageIndex = i;
            await drawImage(canvas);
            var imageData = $('.edit_photo_canvas')[0].toDataURL("image/png");
...
async function drawImage(canvas) {
   ...
   var tempVar = images[imageIndex].width < images[imageIndex].height ? images[imageIndex].width : images[imageIndex].height;
   tempVar = tempVar / currentScale[imageIndex];
   var tempRatio = tempVar / 730;
   imageX = ((images[imageIndex].width - tempVar) / 2) - (newX[imageIndex] * tempRatio);
   imageY = ((images[imageIndex].height - tempVar) / 2) - (newY[imageIndex] * tempRatio);

   ctx.drawImage(images[imageIndex],
      imageX, imageY, tempVar, tempVar,
      0, 0, cvs.width, cvs.height);
   original_img[imageIndex] = ctx.getImageData(0, 0, cvs.width, cvs.height);
   await cammanFilter(camanIndex[imageIndex]);
};
...
return new Promise(function (resolve, reject) {
   switch (index) {
      case 0:
         camanInstance[imageIndex] = Caman('.edit_photo_canvas', function () {
            this.saturation(6);
            this.brightness(3);
            this.channels({
               red: 3,
